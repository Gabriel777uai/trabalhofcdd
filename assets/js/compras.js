...
fetch(`${url_base}api/v1/products/lastcode`, {
  method: "GET",
  headers: {
  ...
  document.getElementById('code_last').innerHTML = data.result;
})
...
window.finalizarCompra = async function () {
  ...
  if (!produtoNome || !qtd || !fornecedorId) {
    Swal.fire(
      "Ops",
      "Preencha todos os campos obrigatórios (Fornecedor, Produto e Quantidade).",
      "warning",
    );
    return;
  }
  ...
  try {
    ...
    Swal.fire(
      "Erro",
      "Ocorreu um problema ao enviar os dados para o servidor.",
      "error",
    );
  }
};
