...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
    ...
    if (userData) {
      ...
      document.getElementById("dataNasc").value = userData?.ia_datanascimento || "";
    }
  } catch (error) {
...
document.getElementById('btnCollapse').addEventListener('click', function () {
  ...
  icon.innerHTML = icon.innerHTML === '<i class="bi bi-chevron-down"></i>' ? '<i class="bi bi-chevron-up"></i>' : '<i class="bi bi-chevron-down"></i>';
})
...
document.getElementById('alterarPass').addEventListener('submit', async function (event) {
  event.preventDefault();
