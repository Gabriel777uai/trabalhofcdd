...
function renderList(compras) {
    ...
    compras.forEach((compra) => {
        ...
        if (compra.f_cancelado) {
            buttons.push(`<button title="Visualizar compra" onclick="visualizarCompra(${compra.id})" class="btn-icon-premium btn-view ms-1"><i class="bi bi-eye"></i></button>`);
            ...
            buttons.push(`<button title="Cancelar compra" onclick="cancelarCompra(${compra.id})" class="btn-icon-premium btn-cancel ms-1"><i class="bi bi-x-lg"></i></button>`);
            buttons.push(`<button title="Visualizar compra" onclick="visualizarCompra(${compra.id})" class="btn-icon-premium btn-view ms-1"><i class="bi bi-eye"></i></button>`);
            buttons.push(`<button title="Dar entrada no estoque (Finalizar recebimento)" onclick="entradaEstoqueCompra(${compra.id})" class="btn-icon-premium ms-1" style="background-color: #e3f2fd; color: #0288d1; border: none; border-radius: 10px; width: 38px; height: 38px; transition: all 0.2s ease;"><i class="bi bi-box-seam"></i></button>`);
            buttons.push(`<button title="Fechar compra" onclick="finalizarCompraItem(${compra.id})" class="btn-icon-premium btn-finish ms-1"><i class="bi bi-check-lg"></i></button>`);
...
window.visualizarCompra = async function(id) {
    try {
        ...
        const dataFormatada = new Date(compra.dt_cadastro).toLocaleDateString("pt-BR");
      ...
      visualizacao.classList.add("active-view");
        ...
        console.error(e);
        Swal.fire("Erro", "Não foi possível carregar os detalhes da compra.", "error");
    }
}
...
window.entradaEstoqueCompra = async function(id) {
    Swal.fire({
        ...
        if(result.isConfirmed) {
            try {
                const userName = localStorage.getItem("usuario") || "Anônimo";
                await fetch(`${url_base}api/v1/compras/update/compra/${id}`, {
                    method: 'PUT',
                    headers: { 
                        "Content-Type": "application/json",
                        Authorization: `Bearer ${localStorage.getItem("acessToken")}` 
                ...
                Swal.fire("Sucesso!", "A entrada no estoque foi realizada.", "success").then(() => {
                    location.reload();
                });
            } catch(e) {
                console.error(e);
                Swal.fire("Erro", "Ocorreu um problema ao dar entrada na compra.", "error");
            }
        }
    });
}
      ...
      if (!hasVisible && rows.length > 0) {
          if (!isExistingEmptyMsg) {
              ...
              if (isExistingEmptyMsg.querySelector('h5')) {
                 isExistingEmptyMsg.querySelector('h5').innerText = `Nenhum resultado para "${e.target.value}"`;
              }
          }
      } else {
          if (isExistingEmptyMsg) {
