...
async function getDataApi(url) {
  ...
  for (let i=0;i<data.length;i++) {
    ...
    <img src="${
          data[i].ia_imagenslink || "https://via.placeholder.com/400x300?text=Sem+imagem"
    ...
    <div class="card-body d-flex flex-column">
      ...
      <button class="btn btn-sm btn-primary" data-id="${data[i].ia_idproduto}">Visualizar</button>
      <button class="btn btn-sm btn-warning" data-id="${data[i].ia_idproduto}">Editar</button>
      <button class="btn btn-sm btn-danger" data-id="${data[i].ia_idproduto}">Excluir</button>
      </div>
    </div>
    `)
